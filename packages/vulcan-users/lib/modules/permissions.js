...
Users.restrictDocuments = function({ user, collection, documents }) {
  return tracer.trace('restrictDocuments', () => {
    const restrictDocumentsSpan = tracer.scope().active();
    restrictDocumentsSpan.setTag('collection', collection._name);

    const check = get(collection, 'options.permissions.canRead');
    let readableDocuments = documents;
    if (check) {
      readableDocuments = tracer.trace('restrictDocuments.documentLevel', () => {
        return documents.filter(comment => {
    ...
    const restrictedDocuments = tracer.trace('restrictDocuments.fieldLevel', () => {
      return Users.restrictViewableFields(user, collection, readableDocuments);
    });
